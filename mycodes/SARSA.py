...
for episode in range(num_episodes):
    ...
    for k in range(len(trajectory)):  # 正反遍历都是一样的，因为没算acc_reward
        ...
        # TD-target是now_score + gamma * qtable[next_state][next_action]
        # 意味着SARSA算法的评估过程是依赖于当前策略的下一个动作next_action，所以策略评估过程是依赖于当前策略target_policy，因此只能是on-policy
        TD_target = now_score + gamma * qtable[next_state][next_action]
...
end_time = time.time()
print("SARSA time:", end_time - start_time)
